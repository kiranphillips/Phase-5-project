...
function Activities ({ setIsLoggedIn, page, loggedInGuestId }) {
  ...
  useEffect(() => {
    setIsLoggedIn(sessionStorage.getActivities("loggedIn"))
    fetch("/activities")
      .then((r) => r.json())
      .then((activities) => page === 'guest' ? setActivities(activities.filter(activities => activities.guest.id === parseInt(loggedInGuestId))) : setActivities(activities));
  }, []);
  ...
  const modalClose = () => {
    setShow(false)
    page === 'guest' ? setActivities([...Activities.filter(activities => activities.guest.id === parseInt(loggedInGuestId))]) : setActivities([...Activities])
    window.location.reload(true)
  ...
  return (
    ...
    {show && <Availabilites closeModal={modalClose} loggedInGuestId={loggedInGuestId} onAddItem={handleAddActivities}/>}
